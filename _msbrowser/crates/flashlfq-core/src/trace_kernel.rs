...
/// One charge state's envelope within an accepted mass hypothesis: the charge, its monoisotopic
/// m/z, the matched-filter response it contributed, how many distinct isotope teeth it showed, and
/// the peaks it matched inside the scoring window.
...
/// Candidate monoisotopic masses for a seed peak: for each charge hypothesis `z_seed`, anchor the
/// seed on the most-abundant averagine tooth `i*` (as the per-charge detector does), walk down to the
/// monoisotope, and take the neutral mass — keeping only those in the plausible top-down band. Each
/// candidate is later scored as a *whole ladder*; enumerating `z_seed` is just how we turn one m/z into
/// the handful of neutral masses it could be the apex of.
fn seed_candidate_masses(
    seed_mz: f64,
    params: &TraceKernelParameters,
) -> Vec<(i32, f64)> {
...
fn emit_mass_hypothesis(
    ...
    for ce in &hyp.charges {
        let peaks = gather_charge_extent(
            engine, ce.mono_mz, ce.charge, &weights, s_lo, s_hi, ppm, claimed, &mut seen,
        );
        if peaks.is_empty() {
            continue;
        }
        ...
        pending.push((
            ChargeSpec { charge: ce.charge, mono_mz: ce.mono_mz, response: ce.response, num_isotopes_observed: ce.num_isotopes_observed },
            peaks,
        ));
    }
...
fn detect_features_multicharge(
    ...
    for (seed_idx, seed) in seeds.iter().enumerate() {
        ...
        // Enumerate candidate neutral masses, score each as a full charge ladder, keep the strongest
        // that clears the multi-charge floor. The candidate list is small (≤ max_charge), and most
        // fail cheaply inside `score_mass_hypothesis` (few/zero charges pass the isotope floor).
        let mut best: Option<MassHypothesis> = None;
        for (z_seed, mono_mass) in seed_candidate_masses(seed_mz, params) {
            let hyp = score_mass_hypothesis(engine, mono_mass, z_seed, params, &ppm, &claimed, &window);
